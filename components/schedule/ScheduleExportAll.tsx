...
interface ScheduleExportAllProps {
  lessons: Lesson[];
...
const ScheduleExportAll = forwardRef<HTMLDivElement, ScheduleExportAllProps>(
  ({ lessons, weekRange, generatedAt }, ref) => {
    const getLesson = (shift: Shift, day: Day, period: number): Lesson | null => {
      return (
        lessons.find(
    ...
    return (
      ...
      >
        <div className="flex items-center justify-between gap-8 mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-[var(--foreground-secondary)]">MrErgashev</p>
            </div>
          </div>
        ...
        <div
          ...
          style={{
            gridTemplateColumns: '130px repeat(5, minmax(150px, 1fr))',
            gap: '12px',
          }}
        >
          <div />
          ...
          {SHIFTS.map((shiftData) => {
            const badge = shiftBadgeStyles[shiftData.shift];
            return (
              <React.Fragment key={`shift-${shiftData.shift}`}>
                <div
                  className="flex items-center gap-3 px-4 py-2 rounded-2xl bg-[var(--background)]/70 border border-[var(--glass-border)]"
                ...
                >
                  ...
                  <span className="text-xs text-[var(--foreground-secondary)]">
                    {shiftData.times.map((t) => t.time).join(' / ')}
                  </span>
                </div>

                {shiftData.times.map((time: TimeSlot) => (
                  <React.Fragment key={`${shiftData.shift}-${time.period}`}>
                    ...
                    {DAYS.map((day) => {
                      const lesson = getLesson(shiftData.shift, day.value, time.period);
                      return (
                        <div key={`${shiftData.shift}-${day.value}-${time.period}`}>
                          {lesson ? (
                            <div className="rounded-xl border border-[var(--glass-border)] bg-[var(--background-secondary)] p-3 min-h-[110px] shadow-sm">
                            ...
                            <div className="rounded-xl border border-dashed border-[var(--glass-border)] bg-[var(--background)]/50 p-3 min-h-[110px] flex items-center justify-center text-xs text-[var(--foreground-secondary)]">
                              Bo&apos;sh
                            </div>
                          )}
                        </div>
...
ScheduleExportAll.displayName = 'ScheduleExportAll';

export default ScheduleExportAll;
